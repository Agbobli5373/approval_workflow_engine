...
public record DelegationActor(
    UUID userId,
    String subject,
    Set<String> roles
) {

    public static DelegationActor fromJwt(Jwt jwt) {
        String subject = jwt.getSubject();
        UUID userId = resolveUserId(jwt, subject);
        return new DelegationActor(userId, subject, extractRoles(jwt));
    }

    public boolean workflowAdmin() {
        return roles.contains("WORKFLOW_ADMIN");
    }
