...
class RequestLifecycleApiTest {
    ...
    void createAndReadOwnRequest() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/requests/" + requestId).header("Authorization", "Bearer " + requestorToken))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.id").value(requestId))
            .andExpect(jsonPath("$.status").value("DRAFT"));

        mockMvc.perform(get("/api/v1/requests/" + requestId).header("Authorization", "Bearer " + approverToken))
    ...
    void listSupportsPaginationFilteringAndOwnership() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/requests?page=0&size=2")
                .header("Authorization", "Bearer " + requestorToken))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.items.length()").value(2))
            .andExpect(jsonPath("$.page.totalElements").value(3));

        mockMvc.perform(get("/api/v1/requests?requestType=TRAVEL")
                .header("Authorization", "Bearer " + requestorToken))
            .andExpect(status().isOk())
